...
public final class Sequence {
    ...
    private static final int SEQUENCE_SHIFT = 3;
    ...
    private static final void nextSequence(SequenceOnce sequenceOnce) {
        ...
        sequenceOnce.sequenceString = addFormat(tc, 3);
    }

    private static void validServerid() {
        if (Sequence.serverid.compareAndSet(0, 1)) {
    ...
    static {
        CODE_STRING = TransforCode64String.CODE_STRING;
        LENGTH = (short)CODE_STRING.getLength();
        EMPTY = CODE_STRING.serialize(0L)[0];
        SEQUENCE_MAX = (int)Math.pow((double)LENGTH, 3.0D) - 1;
